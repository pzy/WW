...
public class WW2x1 extends WWBaseWidget {
	...
	protected void setWidget(WWHandler ha, Bitmap b) {
		...
		updateViews = new RemoteViews(context.getPackageName(), R.layout.widget2x1);
		manager = AppWidgetManager.getInstance(context);
		widget = new ComponentName(context, WW2x1.class);
		PendingIntent pendingShowActivityIntent;
		Intent intent = new Intent(context, WW2x1.class);
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals("android.appwidget.action.APPWIDGET_UPDATE")) {
			...
			if(intent.hasExtra("h")) {
				...
				Thread t=new Thread(new WW2x1(context,h));
				t.start();
			}
